...
def list_oper():
    ...
    print(lst[:3]) # 시작 경계 생략 > 처음부터라는뜻
    print(lst[:])  # 시작경계 끝경계 생략 > 전체

    copy = lst[:] #슬라이싱을 이용한 리스트 전체의 복사
    print(copy)
    print(copy is lst) # 슬라이싱 > 새 객체가 생성 (별개의 객체)
    ...
    if "Java" in lst:
        print("INDEX:", lst.index("Java")) # 업슨ㄴ 객체 검색시 ValueError
    ...
    # 슬라이싱 이용 삭제
    lst[0:2] =[] # 슬라이싱 범위에 빈 리스트를 할당하면 삭제
    print(lst)
    # 슬라이싱 이용한 삽입
